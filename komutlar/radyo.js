...
const radyo = {
    ...
    vergin: "https://playerservices.streamtheworld.com/api/livestream-redirect/VIRGIN_RADIOAAC.aac?dist=onlineradiobox ",
            ...
            message.member.voiceChannel.join().then(connection => {
              var dispatcher = connection.playStream(radyo.maxlounge);
               message.reply("🎧 | **Başarılı! `MaxLounge` çalınıyor. :musical_keyboard:**").then(m => m.delete(10000)).catch(console.error);
                                ...
                                } else if (args[0].toLowerCase() === "verginradyo" || args[0] === "31") {
            message.member.voiceChannel.join().then(connection => {
              var dispatcher = connection.playStream(radyo.maxlounge);
               message.reply("🎧 | **Başarılı! `VerginRadyo` çalınıyor. :musical_keyboard:**").then(m => m.delete(10000)).catch(console.error);
            })
